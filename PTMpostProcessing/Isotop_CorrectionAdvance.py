...
def isotop(folder, targetFile, madFIle):
    ...
    Tdic, Ddic = separateTargetAndDecoy(targetFile, folder=folder1)

    Seq2MassNegDic = createDic(Tdic, key=7, val = int(seconslastElement), header =True)
    ...
    for lists in MainList:
        for list2 in lists:
            deltaPeptide = list2[1].split("\t")[int(seconslastElement + 1)]
            ...
            wfile.write(
                list2[1] + "\t" + finalSeqMassTag + "\t" + str(list2[0]) + "\t" + list2[2] + "\n")
