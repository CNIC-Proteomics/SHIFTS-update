...
def listFiller(window):
    ##### if odd window ####
    if not window % 2 == 0:
        remainder = int(window / 2)
        firstFiller = remainder
        startPoint = remainder + 1
        LastFiller = remainder
    else:
        remainder = window / 2
        firstFiller = remainder - 1
        startPoint = remainder
        LastFiller = remainder

    return firstFiller, startPoint, LastFiller
...
def check(mass, n):
    if mass < 0.0:
        ...
        testout1 = str("{:.6f}".format(testout1))
...
def getIndex(filename):
    for line in open(filename):
        if line!="\n":
            splits = line.split("\t")
            secondLast = splits.index(splits[-2])
            break
    return secondLast
