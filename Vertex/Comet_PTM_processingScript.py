                    ...
                    if -500 < delta_m < 500.0: ##### selecting the delta mass within 500 Da range
                        if str(splits[ver2col[5]]).strip("\" ") == "1":  # and scan_fn not in SE1_dic: selecting only search engine rank one PSMs
                            ...
                            if CorrectedXcor >= Xcorthershold and -0.1 < scanDelta_MZ < 0.1:  ##### new charge condition. search engine rank fixed
                                ...
                                if "DECOY" not in splits[proteinFastaColumn].strip("\" ") and "INV" not in splits[proteinFastaColumn].strip("\" ")and "XXX_" not in splits[proteinFastaColumn].strip("\" ") \
                                        and "_INV_" not in splits[proteinFastaColumn].strip("\" "): #### checking for target PSMs
                            ...
                            if Continue:
                                ...
                                ln += [str(scanExp_MH)]
                                if "DECOY_" not in splits[proteinFastaColumn].strip("\" ") and "INV_" not in splits[proteinFastaColumn].strip("\" ")and "XXX_" not in splits[proteinFastaColumn].strip("\" ")\
                                        and "_INV_" not in splits[proteinFastaColumn].strip("\" "):
                                    label = "Target"
                                else:
                                    label = "Decoy"
